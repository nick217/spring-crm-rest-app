...
public class DemoSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		// Why disable CSRF?
		//
		// Spring Security 5 has CSRF enabled by default. You would need to send
		// over CSRF tokens.
		// However, CSRF generally does not apply for REST APIs. CSRF protection
		// is a request that could be processed by a browser by normal users.
		// If you are only creating a REST service that is used by non-browser
		...
		//
		// For our application, we would like avoid the use of cookies for
		// sesson tracking. This should force the REST client
		// to enter user name and password for each request. However, this is
		// not always the case depending on the REST client / browser
		// you are using. Your mileage will vary here (for example, this doesn't
		// work in Eclipse embedded browser).
		//
		// For more details, see this link
